...
const searchRecipes = async (req, res) => {
  ...
  try {
    // Escape special characters in the search query to prevent invalid regex errors
    const escapedSearchKey = searchKey.replace(/[-[\]/{}()*+?.\\^$|]/g, "\\$&"); // Escape special characters

    // Use a case-insensitive regular expression to search for recipes by title
    const recipes = await Recipe.find({
      title: { $regex: new RegExp(escapedSearchKey, "i") },
    });
